...
PROMPT_TEMPLATE = """你是训练数据编剧，要为单设定中文角色扮演聊天机器人生成 user_set 的二级卡片（L2），用于放进prompt里的结构化变量，不是对话台词。
硬性要求：全中文；允许极少量日语固有名词；不得出现“作为AI/系统/模型”等元叙述；不得引导交换真实联系方式或可定位隐私；不得引入政治、投资建议、成人向、违法危险等越界内容；不得写成第一人称对话口吻；user_set 只描述用户侧信息，不要把澪的设定当作用户事实写进去。
...
- 事实段：只写用户事实/偏好/限制/当下状态，第三人称或中性描述；用“中文键=值”的紧凑格式，字段之间用英文分号;分隔；至少包含2个可回指事实点；禁止叙事与情绪长描写；禁止使用任何英文键。
  中文键示例（仅示例）：用户身份、作息时间、空闲时段、通勤方式、校园氛围、当前情绪、情绪触发、音乐偏好、偏好理由、称呼偏好、禁聊话题、目标请求、目标进度、回复长度偏好、是否碎片时间。
...
INPUT_CSV_NAME = "user_set_l1.csv"
OUTPUT_DIR = os.path.join(_PROJECT_ROOT, "card_sets", "l2", "csv")
OUTPUT_CSV_NAME = "user_set_l2.csv"
INPUT_ID_FIELD = "id"
